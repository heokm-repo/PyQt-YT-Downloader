"""
범용 다운로드 핸들러
yt-dlp를 사용하여 YouTube 및 기타 모든 지원 사이트의 영상을 다운로드
(기존 youtube_handler.py에서 범용화)
"""
import os
import re
...
# =====================================================================
# YouTube 전용 URL 유틸리티 (YouTube URL일 때만 사용)
# =====================================================================

def _sanitize_url(url, prefer_playlist=False):
    """
    URL 정제 및 플레이리스트/비디오 구분 로직
    YouTube URL일 때만 특수 처리, 그 외는 그대로 반환
    """
    if not url: return "", False
    
    # YouTube URL이 아니면 그대로 반환
...
def has_video_and_list(url):
    """URL에 v와 list가 동시에 존재하는지 확인 (YouTube 전용)"""
    if not url: return False
    
    # YouTube URL이 아니면 항상 False
    if not is_youtube_url(url):
        return False
...
def fetch_metadata(url, settings=None):
    """
    영상 메타데이터 조회 (범용)
    반환되는 dict에 'extractor' 필드를 포함하여 사이트 식별
    """
    # YouTube URL인 경우에만 sanitize 수행
    ...
    try:
        ...
        # extractor 정보 추출 (yt-dlp가 자동으로 제공)
        extractor = info.get('extractor', info.get('extractor_key', 'unknown'))
        if extractor:
        ...
        if 'entries' in info:
            info = info['entries'][0]
            # entries의 첫 항목에서 extractor 다시 추출
            extractor = info.get('extractor', info.get('extractor_key', extractor))
            if extractor:
                extractor = extractor.lower()
...
def _build_advanced_options(settings):
    ...
    # 인앱 로그인 쿠키 파일 사용
    try:
        from gui.windows.login_browser import get_cookie_file_path, cookie_file_exists
        if cookie_file_exists():
            cookie_path = get_cookie_file_path()
            opts['cookiefile'] = cookie_path
            log.info(f"Using cookie file: {cookie_path}")
    except ImportError:
        log.debug("login_browser module not available, skipping cookie support")
    
    # QuickJS JS 런타임 경로 (yt-dlp 서명 풀기에 필요)
    try:
        from utils.bin_manager import get_quickjs_path
