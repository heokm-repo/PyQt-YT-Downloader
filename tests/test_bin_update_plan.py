...
class BinUpdatePlanTests(unittest.TestCase):
    def test_initial_update_results_defaults_to_managed_binaries(self):
        self.assertEqual(initial_update_results(), {"yt-dlp": False, "ffmpeg": False})
        self.assertEqual(MANAGED_UPDATE_BINARIES, ("yt-dlp", "ffmpeg"))
    ...
    def test_selected_update_binaries_defaults_when_no_selection(self):
        self.assertEqual(selected_update_binaries(None), ["yt-dlp", "ffmpeg"])
    ...
    def test_should_check_after_requires_elapsed_interval(self):
        now = datetime(2026, 7, 1, 12, 0, 0)

        self.assertFalse(should_check_after((now - timedelta(hours=11)).isoformat(), 12, now))
        self.assertTrue(should_check_after((now - timedelta(hours=13)).isoformat(), 12, now))

    def test_should_check_after_checks_when_missing_or_invalid(self):
        now = datetime(2026, 7, 1, 12, 0, 0)

        self.assertTrue(should_check_after(None, 12, now))
        self.assertTrue(should_check_after("", 12, now))
        self.assertTrue(should_check_after("not-a-date", 12, now))
        self.assertTrue(should_check_after(123, 12, now))
