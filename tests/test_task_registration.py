...
class FakeScheduler:
    ...
    def add_task(self, priority, task_id, url, settings):
        self.added.append((priority, task_id, url, settings))
...
class TaskRegistrationTests(unittest.TestCase):
    ...
    def test_register_download_task_registers_widget_stores_task_and_enqueues(self):
        ...
        self.assertIs(task.settings, scheduler.added[0][3])
        ...
        self.assertEqual(
            scheduler.added,
            [(NORMAL_TASK_PRIORITY, 12, "https://example.test/watch?v=12", task.settings)],
        )
