...
class BinReleaseFetchTests(unittest.TestCase):
    ...
    def test_strict_release_check_raises_on_request_error(self):
        with patch.object(
            bin_release_fetch.requests,
            "get",
            side_effect=bin_release_fetch.requests.RequestException("network down"),
        ), self.assertRaises(bin_release_fetch.ReleaseCheckError):
            bin_release_fetch.check_latest_github_release_strict(
                "https://example.test/releases/latest",
                "tool",
                lambda data: ("v1", "url", "sha256:" + "a" * 64),
                "missing",
    ...
    def test_strict_release_check_rejects_missing_digest(self):
        response = FakeReleaseResponse({"tag_name": "v1"})
