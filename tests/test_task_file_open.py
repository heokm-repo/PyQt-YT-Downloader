...
class TaskFileOpenTests(unittest.TestCase):
    ...
    def test_open_output_folder_selects_existing_file(self):
        selected_paths = []
        opened_paths = []
        with tempfile.TemporaryDirectory() as tmpdir:
            file_path = Path(tmpdir) / "video.mp4"
            ...
            result = open_output_folder(str(file_path), opened_paths.append, selected_paths.append)

        self.assertEqual(result.status, OpenFolderStatus.OPENED)
        self.assertTrue(result.select_file)
        self.assertEqual(selected_paths, [str(file_path)])
    ...
    def test_open_output_folder_opens_existing_parent_for_missing_file(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = open_output_folder(str(missing_path), opened_paths.append, selected_paths.append)
    ...
    def test_open_output_folder_reports_errors(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            result = open_output_folder(str(missing_path), raise_error, lambda _path: None)
