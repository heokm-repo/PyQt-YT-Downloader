...
class SettingsControlsTests(unittest.TestCase):
    ...
    def test_compatibility_mode_limits_format_combo_to_mp4_and_mp3(self):
        combo = create_format_combo("webm", "Video", "Audio")

        set_compatibility_format_mode(combo, True)

        self.assertEqual(combo.currentText(), "mp4")
        enabled_formats = {
            combo.itemText(index)
            for index in range(combo.count())
            if combo.model().item(index).isEnabled()
        }
        self.assertEqual(enabled_formats, {"mp4", "mp3"})

        set_compatibility_format_mode(combo, False)
        enabled_formats = {
            combo.itemText(index)
            for index in range(combo.count())
            if combo.model().item(index).isEnabled()
